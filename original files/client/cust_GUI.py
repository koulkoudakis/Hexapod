	...
	while True:
		try:
			...
			try:
				...
				cv2.putText(source,('Distance(cm): %s' %ULT_DIST),(370, 440), font, 0.5, (128, 255, 128),1, cv2.LINE_AA)
...
def Info_receive():
	global CPU_TEP,CPU_USE,RAM_USE,ULT_DIST
	HOST = ''
	...
	while 1:
		try:
			...
			CPU_TEP,CPU_USE,RAM_USE,ULT_DIST= info_get
...
def loop():					  #GUI
	...
	while True:
		...
		Btn_Fun4 = tk.Button(root, width=10, text='FaceTracking',fg=color_text,bg=color_btn,relief='ridge')
		Btn_Fun4.place(x=285,y=445)
		root.bind('<KeyPress-z>', call_WatchDog)
		Btn_Fun4.bind('<ButtonPress-1>', call_WatchDog)

		Btn_Fun5 = tk.Button(root, width=10, text='Distance',fg=color_text,bg=color_btn,relief='ridge')
		Btn_Fun5.place(x=370,y=445)
		root.bind('<KeyPress-z>', call_WatchDog)
		Btn_Fun5.bind('<ButtonPress-1>', call_WatchDog)

		Btn_Fun6 = tk.Button(root, width=10, text='Laser',fg=color_text,bg=color_btn,relief='ridge')
